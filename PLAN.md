...
- Job 02 (git publication) is complete (2026-05-13): `.gitignore`, `.gitattributes`, root README, packages README all in place; initial commit `60c04e4` (142 files); remote origin set to `https://github.com/nofaniel/Crabtya.git`. Push pending user instruction.
- Git publication/repo polish is intentionally last for the Crabtya v1 release flow, but every earlier v1 Job should leave docs, package boundaries, and source ownership cleaner so Job 02 is mostly final assembly rather than archaeology.
...
- The intended private GitHub remote is `https://github.com/nofaniel/Crabtya.git`. Add/init/push the project there as soon as Job 02 confirms the repository is safe for source control and no local game/runtime/private artifacts would be accidentally committed.
...
| 5 | 09 | In progress (2026-05-13, runtime proof pending) | High | Medium | Surface Verification | `docs/development/jobs/job-09-assetbundle-proof.md` | Prove `content.assetBundles` end to end with a real Unity 6000.2.15f1 bundle and document the mod-maker build recipe. | Bundle loads + asset retrieves + visible effect, with success and failure paths logged. |
...
| 10 | 02 | Done (2026-05-13) | High | Medium | Publication | `docs/development/jobs/job-02-git-initial-readiness.md` | Prepare repo for initial git/GitHub publication with README, ignore rules, and release clarity. | Root README and `.gitignore` ready; initial commit 60c04e4 (142 files); remote origin set to https://github.com/nofaniel/Crabtya.git. Push pending user instruction. |
...
Use the available logo files (`logo-L.png`, `logo-M.png`, `logo-S.png`) for README/GitHub presentation, release-page imagery, and any repo social preview guidance. The target private GitHub repository is `https://github.com/nofaniel/Crabtya.git`; once `.gitignore`, README, and publication risk review are complete, initialize/add the project to git and set that remote as the destination unless the user redirects.
